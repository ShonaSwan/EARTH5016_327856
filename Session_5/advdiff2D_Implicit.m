...
%***** RUN 2D ADVECTION DIFFUSION MODEL (Implicit) *******
% clear workspace
...
ADVN = 'UpW1';    % advection scheme ('UPW1', 'CFD2', 'UPW3', 'WENO5')
TINT = 'BE1';      % time integration scheme ('BE1','CN2')
...
while t <= tend
    ...
    % assemble coefficient matrix for space-dependent coefficients
     JJ = []; II = []; AA= [];  % initialise index and value lists
   
     jj = Map(ind3_z(2:end-1),ind3_x(2:end-1));

     a_center = (+2*k0/dx^2 + 2*k0/dz^2) * ones(size(jj));
     a_top    = (-v0/(2*dz) - k0/dz^2) * ones(size(jj));
     a_bottom = (+v0/(2*dz) - k0/dz^2) * ones(size(jj));
     a_left   = (-u0/(2*dx) - k0/dx^2) * ones(size(jj));
     a_right  = (+u0/(2*dx) - k0/dx^2) * ones(size(jj));

     ii1 = Map(ind3_z(1:end-2),ind3_x(2:end-1));   %top
     ii2 = Map(ind3_z(3:end)  ,ind3_x(2:end-1));   %bottom
     ii3 = Map(ind3_z(2:end-1),ind3_x(1:end-2));   %left
     ii4 = Map(ind3_z(2:end-1),ind3_x(3:end));     %right 
     ...
     AA = [AA;a_center(:)]; %central
     AA = [AA;a_top(:)];     %top 
     AA = [AA;a_bottom(:)];  %bottom 
     AA = [AA;a_left(:)];    %left
     AA = [AA;a_right(:)];   %right


     Ax = sparse(JJ,II,AA,N,N);

    switch TINT
       
        case 'BE1'  % Backward Euler
             A = At + Ax ;       % Assemble coefficient matr
             b = At * T(:);      % Forcing vector (explicit component)
             T = (A\b).';       % Solve system
        
             T = reshape(T(Map(:)), Nz, Nx);

             %W = full(reshape(SOL(MapW(:))        ,Nz+1,Nx+2));  % matrix z-velocity

        case 'CN2'  % Crank-Nicolson
             A = At + Ax / 2;   % Assemble coefficient matrix (implicit part)
             b = (At - Ax / 2) * T(:);  % Forcing vector (explicit part)
             T = (A\b).';       % Solve system
